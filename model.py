...
def apply_rotary_emb(
    ...
    xq_ = torch.view_as_complex(xq.float().reshape(*xq.shape[:-1], -1, 2)).to(params.device)
    xk_ = torch.view_as_complex(xk.float().reshape(*xk.shape[:-1], -1, 2)).to(params.device)
    freqs_cis = reshape_for_broadcast(freqs_cis, xq_)
    xq_out = torch.view_as_real(xq_ * freqs_cis).flatten(3).to(params.device)
    xk_out = torch.view_as_real(xk_ * freqs_cis).flatten(3).to(params.device)
...
class Transformer(nn.Module):
    ...
    def generate(
        ...
        for i in range(max_gen_len):
            ...
            token_ids_list = next_token.squeeze().tolist()
            print(self.tokenizer.decode(token_ids_list), end='', flush=True)
